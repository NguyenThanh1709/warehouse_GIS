...
$data = array(
  'pageTitle' => 'Thêm đơn nhập hàng',
...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
  ...
  if (empty($body['code'])) {
    $errors['code'] = "Vui lòng điền mã đơn nhập!";
  }
  ...
  if ($body['suppliers'] == 0) {
    $errors['suppliers'] = "Vui lòng chọn nhà cung cấp!";
  }

  if (empty($listCartProduct)) {
    $errors['cart'] = "Vui lòng chọn sản phẩm nhập hàng!";
  }
  ...
  if (empty($errors)) {
    ...
    if ($insertStatus) {
      ...
      delete_cart();
      redirect(_WEB_HOST_ROOT . '?module=import');
    } else {
        ...
        <form method="POST">
          <div class="row">
            <div class="col-6 form-group">
              <label for="code">Mã đơn nhập (*)</label>
            ...
            <div class="col-6 form-group">
              <label for="phone">Nhà cung cấp (*)</label>
              <select name="suppliers" id="" class="form-control">
                <option value="0">----Chọn nhà cung cấp----</option>
                <?php foreach ($listSupplier as $item): ?>
                  <option <?php echo (!empty(old('suppliers', $old_data)) && old('suppliers', $old_data) == $item['id']) ? 'selected' : null ?> value="<?php echo $item['id'] ?>"><?php echo $item['name'] ?></option>
                <?php endforeach; ?>
              </select>
              <?php echo form_error('suppliers', $errors) ?>
            </div>
            <div class="col-6">
                  ...
                  <tr>
                    <td colspan="5" class="text-right">
                      <a href="?module=import&action=deleteCart" onclick="return confirm('Bạn chắc chắc xoá sản phảm')" class="btn btn-sm btn-outline-danger mb-3" fdprocessedid="6osz9j">
                        ...
                        Xoá giỏ hàng
                      </a><br>Tổng đơn nhập: <strong id="total"><?php echo number_format(get_total_cart(), 0, '', ',') . 'đ' ?></strong>
                    </td>
                  </tr>
                </tfoot>
              </table>
