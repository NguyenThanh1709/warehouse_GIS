...
$productDetail = firstRaw("SELECT * FROM `tbl_sanpham` WHERE `id`='$id'");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  if (empty($errors)) {
    ...
    $updateStatus = update('tbl_sanpham', $data, "`id`='$id'");

    //Kiểm tra cập nhật thành công hay thất bại
    if ($updateStatus) {
      unset($_SESSION['old_data']);
      $_SESSION['msg'] = "Đã cập nhật sản phẩm mới thành công!";
      $_SESSION['msg_style'] = "success";
      redirect(_WEB_HOST_ROOT . "?module=products&action=edit&id=$id"); //Chuyển hướng
    } else {
      $_SESSION['msg'] = "Đã xảy ra lỗi trong quá trình cập nhật người dùng! Thử lại sau!";
      ...
      <div class="card-body">
        <form method="POST" enctype="multipart/form-data">
          ...
          <div class="form-group">
            ...
            <textarea name="note" class="form-control" id="intro" cols="30" rows="5"><?php echo old('note', $old_data) ?></textarea>
          </div>
          ...
          <div class="row">
            <div class="col-3">
              <div class="form-group">
                ...
                <input class="form-control" value="<?php echo old('unit', $old_data) ?>" type="text" name="unit" id="unit">
            ...
            <div class="col-6">
              <div class="form-group">
                <label for="">Ảnh đã tải lên</label> <br>
                <a href="<?php echo !empty(old('thumbnail', $old_data)) ? old('thumbnail', $old_data) : 'http://via.placeholder.com/80X80' ?>" data-fancybox="gallery">
                  <img width="200" height="150" src="<?php echo !empty(old('thumbnail', $old_data)) ?  old('thumbnail', $old_data) : 'http://via.placeholder.com/80X80' ?>" alt="">
                </a>
              </div>
            </div>
          </div>
