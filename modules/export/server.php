...
if (isset($_POST['keyword'])) {
  ...
  if (!empty($listProduct)) {
    ...
    foreach ($listProduct as $item) {
      ...
      $strHTML .= '<td>' . number_format($item['import_price'], 0, '', ',') . 'đ / ' . $item['unit'] . '</td>';
      $strHTML .= '<td><input type="number" class="form-control" min="1" value="1" id="qty-' . $item['id'] . '"></td>';
...
if (isset($_POST['id_product'])) {
  ...
  add_cart('tbl_sanpham', "`id`='$id'", $id, 'import_price', $qty);
  ...
  // echo json_encode($listProductCart, $str);
  // die();
