...
$data = array(
  'pageTitle' => 'Thông tin chi tiết xuất hàng',
...
$listCartProduct = getRaw("SELECT `tbl_chitiet_xuathang`.*, `tbl_sanpham`.name as name_product, `tbl_sanpham`.thumbnail as thumbnail, `tbl_sanpham`.export_price as export_price
                             FROM `tbl_chitiet_xuathang` INNER JOIN `tbl_sanpham` 
                             ON `tbl_sanpham`.id = `tbl_chitiet_xuathang`.id_product 
                             WHERE `tbl_chitiet_xuathang`.id_export = '$id'");
...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
  ...
  if (empty($errors)) {
    ...
    $updateStatus = update('tbl_xuathang', $data, "`id`='$id'");
    if ($updateStatus) {
      $_SESSION['msg'] = "Đã cập nhật đơn hàng thành công!";
      $_SESSION['msg_style'] = "success";
      redirect(_WEB_HOST_ROOT . '?module=export&action=edit&id=' . $id);
    } else {
        ...
        <div class="row">
          <div class="col-6">
            <div class="form-group d-flex align-items-center">
              ...
              <span class="mx-2"><strong>Mã đơn xuất: </strong><?php echo $exportDetail['code'] ?></span>
            </div>
            <div class="form-group d-flex align-items-center">
              ...
              <span class="mx-2"><strong>Kho hàng: </strong><?php echo $exportDetail['name_warehouse'] ?></span>
            </div>
            <div class="form-group d-flex align-items-center">
              ...
              <span class="mx-2"><strong>Nhà cung cấp: </strong><?php echo $exportDetail['name_customer'] ?></span>
            </div>
            <div class="form-group d-flex align-items-center">
              ...
              <span class="mx-2"><strong>Nhân viên: </strong><?php echo $exportDetail['name_employee'] ?></span>
            </div>
          </div>
          <div class="col-6">
            <div class="form-group d-flex align-items-center">
              ...
              <form method="post" class="mx-2 d-flex align-items-center">
                <span>
                  <select name="status" id="" class="form-control btn-sm w-100">
                    <option <?php echo $exportDetail['status'] == 1 ? "selected" : null ?> value="1">Đã thanh toán</option>
                    <option <?php echo $exportDetail['status'] == 0 ? "selected" : null ?> value="0">Chưa thanh toán</option>
                  </select>
                </span>
            ...
            <div class="form-group d-flex align-items-center">
              ...
              <span class="mx-2"><strong>Ngày xuất: </strong> <?php echo $exportDetail['create_at'] ?></span>
            </div>
            <div class="form-group d-flex align-items-center">
              ...
              <span class="mx-2"><strong>Ghi chú: </strong> <?php echo $exportDetail['note'] ?? 'Không có' ?></span>
            </div>
          </div>
        </div>
      </div>
        ...
        <table class="table table-hover table-bordered mt-2">
          <thead>
            <tr>
              ...
              <th scope="col" width="30%">Giá xuất</th>
          ...
          <tbody class="list-product-cart">
            ...
            ?>
                <tr>
                  ...
                  <td><span><?php echo number_format($item['export_price'], 0, '', ',') . 'đ' ?></span></td>
          ...
          <tfoot>
            <tr>
              <td colspan="6" class="text-right">
                </a><br>Tổng đơn xuất: <strong id="total"><?php echo number_format($exportDetail['total'], 0, '', ',') . 'đ' ?></strong>
              </td>
            </tr>
          </tfoot>
        </table>
        <a href="?module=export" class="btn btn-danger">Quay lại</a>
      </div>
    </div>
  </div>
